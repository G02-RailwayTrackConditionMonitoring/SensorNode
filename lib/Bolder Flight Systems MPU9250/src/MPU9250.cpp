...
int MPU9250::begin(){
  ...
  // if((whoAmI() != 113)&&(whoAmI() != 115)){ //should be 112 i think
  //  return -5;
  // }
  // enable accelerometer and disable gyro
...
int MPU9250::setDlpfBandwidth(DlpfBandwidth bandwidth) {
  ...
  _useSPIHS = false;
  switch(bandwidth) {
    // FIX THIS TO WORK FOR GYRO IF NEEDED LATER
    // case DLPF_BANDWIDTH_BYPASS: {
    //   if(writeRegister(ACCEL_CONFIG2,ACCEL_DLPF_BYPASS) < 0){ // setting accel bandwidth to 184Hz
    //     return -1;
    //   } 
...
int MPU9250FIFO::enableAccelFifo() {
