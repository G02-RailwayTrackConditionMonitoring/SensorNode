...
class MPU9250{
  ...
  protected:
    ...
    // MPU9250 registers
    const uint8_t ACCEL_OUT = 0x3B; // X,Y,Z each 2 Bytes
    ...
    const uint8_t ACCEL_DLPF_BYPASS = 0x08; //*ADDED, disables digital low pass filter for minimal delay allowing 4kHz SR
    ...
    //const uint8_t SMPDIV_MAX_RATE = 0x00; //*ADDED divider for internal clock that controls SR, min should result in max SR
    const uint8_t INT_PIN_CFG = 0x37; //Address, may need to revisit this once deciding how to clear ints
    ...
    //const uint8_t WHO_AM_I = 0x75; //MPU-9250
    const uint8_t WHO_AM_I = 0x70; //MPU-6500
